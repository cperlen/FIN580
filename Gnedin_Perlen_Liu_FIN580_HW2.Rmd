...
test=constructModel(train[,1],p=1,verbose = FALSE,IC = FALSE,ownlambdas = TRUE)
lasso_fit=function(p){
  
}
train_x_lasso=train[]
p_lass=c() #List of lasso predictions
#Getting range of lambdas
lam_1=0
#Get lam_L
cv=cv.glemnet(train,train) #TBU
...
lasso_prediction=function(lam,lookback,index,time_series){
  x_data=time_series[1:index-1]
  y_data=time_series[1:index-1]
}
...
p_lass=c() #List of knn predictions
