...
ps=c(1,2,3)
fit_VAR=function(p){
  model=constructModel(train,p,"Basic",gran=c()
...
pred_var_lasso=function(data,lam,p,ans){
  model=constructModel(data.matrix(data),p=p,"Basic",ownlambdas=TRUE,gran=lam,h=1)
  res=cv.BigVAR(model)
...
for(i in 1:length(ps)){
  ...
  mses_var_lasso[2:nrow(mses_var_lasso),i]=sapply(temp,function(x){pred_var_lasso(rbind(train,test[1:(x),]),lam,i,test[x+1,])})
}
