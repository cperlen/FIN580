...
plot_hmm_train=function(i,n_states){
  ...
  #Daily
  hmm=depmix(Vol~1,family=gaussian(),nstates=n_states,data=daily_data[[i]])
  ...
  #plot(daily_data[[i]]$Date,post_probs$state, type='s', main='True Regimes - Daily', xlab='', ylab='Regime')
  #dev.off()
  ...
  #Weekly
  hmm=depmix(Vol~1,family=gaussian(),nstates=n_states,data=weekly_data[[i]])
  hmm_fit=fit(hmm,verbose=FALSE)
  post_probs=posterior(hmm_fit)
  weekly_states=data.frame(post_probs$state)
   # pdf(paste(currencies[i],"_TR_weekly.pdf",sep=""), onefile=T, paper='A4r')
  #plot(weekly_data[[i]]$Date,post_probs$state, type='s', main='True Regimes - Weekly', xlab='', ylab='Regime')
  #dev.off()
...
for(i in 1:length(currencies)){
  daily_states_2[length(daily_states_2)+1]=plot_hmm_train(i,2)[[1]]
  weekly_states_2[length(weekly_states_2)+1]=plot_hmm_train(i,2)[[2]]
  daily_states_3[length(daily_states_3)+1]=plot_hmm_train(i,3)[[1]]
  weekly_states_3[length(weekly_states_3)+1]=plot_hmm_train(i,3)[[2]]
}
